 ...
 * CSV stream, an indexed SQLite probe, anything else — is this seam's
 * concern; the caller only ever filters and aggregates what comes out, so
 * every reader must yield a superset of the records the filter pipeline
 * would have matched in a full scan, in the file's original row order.
 */
interface LookupSourceReader
{
     ...
     * reports which strategy answered (e.g. `sqlite-index`, `full-stream`)
     * for observability.
     *
